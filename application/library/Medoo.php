...
class
Medoo {
  protected $database_type = 'mariadb';
  protected $charset;
  ...
  protected function init_data(){
    $this->port = Yaf_Application::app()->getConfig()->mysql->port;
    $this->charset = Yaf_Application::app()->getConfig()->mysql->charset;
    $this->database_name = Yaf_Application::app()->getConfig()->mysql->database_name;
    $this->username = Yaf_Application::app()->getConfig()->mysql->username;
    $this->password = Yaf_Application::app()->getConfig()->mysql->password;
    $this->table_prefix = Yaf_Application::app()->getConfig()->mysql->table_prefix;
  }
  ...
  public function __construct($options = null){
    ...
    try{
      ...
      if($this->control_type == 1){
        $this->server = Yaf_Application::app()->getConfig()->mysql->server;
      }else{
        $this->server = Yaf_Application::app()->getConfig()->mysql->server;
      }
      ...
      $pdo = new PDO($dsn, $this->username, $this->password, $this->option);

      if(Yaf_Application::app()->getConfig()->setting->develop){
  ...
  protected function column_quote($string){ return '"' . str_replace('.', '"."', preg_replace('/(^#|\(JSON\))/', '', $string)) . '"'; }
  ...
  protected function fn_quote($column, $string){ return (strpos($column, '#') === 0 && preg_match('/^[A-Z0-9\_]*\([^)]*\)$/', $string)) ? $string : $this->quote($string); }
  ...
  public function insert_update($table, $data, $where = null){
    $data_exist = $this->count($table,$where);
    if($data_exist){
  ...
  public function last_query(){ return end($this->logs); }

  public function log(){ return $this->logs; }
